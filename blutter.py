...
class BlutterInput:
    def __init__(self, libapp_path: str, dart_info: DartLibInfo, outdir: str, rebuild_blutter: bool, create_vs_sln: bool, no_analysis: bool, no_ida_comment:bool):
        self.libapp_path = libapp_path
        self.dart_info = dart_info
        self.outdir = outdir
        self.rebuild_blutter = rebuild_blutter
        self.create_vs_sln = create_vs_sln
        self.no_ida_comment = no_ida_comment
        ...
        if self.no_ida_comment:
            self.name_suffix += '_no_ida_comment'
...
def find_compat_macro(dart_version: str, no_analysis: bool, no_ida_comment: bool):
    macros = []
    ...
    if no_ida_comment:
        macros.append("-DNO_IDA_COMMENT=1")
...
def cmake_blutter(input: BlutterInput):
    ...
    macros = find_compat_macro(input.dart_info.version, input.no_analysis, input.no_ida_comment)
    my_env = None
...
def main_no_flutter(libapp_path: str, dart_version: str, outdir: str, rebuild_blutter: bool, create_vs_sln: bool, no_analysis: bool, no_ida_comment: bool):
    version, os_name, arch = dart_version.split('_')
    dart_info = DartLibInfo(version, os_name, arch)
    input = BlutterInput(libapp_path, dart_info, outdir, rebuild_blutter, create_vs_sln, no_analysis, no_ida_comment)
    build_and_run(input)
    
def main2(libapp_path: str, libflutter_path: str, outdir: str, rebuild_blutter: bool, create_vs_sln: bool, no_analysis: bool, no_ida_comment: bool):
    dart_info = get_dart_lib_info(libapp_path, libflutter_path)
    input = BlutterInput(libapp_path, dart_info, outdir, rebuild_blutter, create_vs_sln, no_analysis, no_ida_comment)
    build_and_run(input)

def main(indir: str, outdir: str, rebuild_blutter: bool, create_vs_sln: bool, no_analysis: bool, no_ida_comment: bool):
    if indir.endswith(".apk"):
        with tempfile.TemporaryDirectory() as tmp_dir:
            libapp_file, libflutter_file = extract_libs_from_apk(indir, tmp_dir)
            main2(libapp_file, libflutter_file, outdir, rebuild_blutter, create_vs_sln, no_analysis, no_ida_comment)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rebuild', action='store_true', help='Force rebuild the Blutter executable')
    parser.add_argument('--vs-sln', action='store_true', help='Generate Visual Studio solution at <outdir>')
    parser.add_argument('--no-analysis', action='store_true', help='Do not build with code analysis')
    # rare usage scenario
    ...
    parser.add_argument('-c', '--no-ida-comment', action='store_true', help='Don\'t dump comments for IDA')
    args = parser.parse_args()

    if args.dart_version is None:
        main(args.indir, args.outdir, args.rebuild, args.vs_sln, args.no_analysis, args.no_ida_comment)
    else:
        main_no_flutter(args.indir, args.dart_version, args.outdir, args.rebuild, args.vs_sln, args.no_analysis, args.no_ida_comment)
